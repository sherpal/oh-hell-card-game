...
object IO {
   ...
   */
  val baseDirectory: String = try {
    // this is only available in the MainProcess.
    Path.join(js.Dynamic.global.myGlobalDirname.asInstanceOf[String], "../")
  } catch {
    case _: Throwable =>
      //Path.join(js.Dynamic.global.selectDynamic("__dirname").asInstanceOf[String], "../../")
    VariableStorage.retrieveGlobalValue("baseDirectory").asInstanceOf[String]
  }
